    ...
    public class Pathfinding
    {
        ...
        private static List<Node> _ImpFindPath(Grid grid, Point startPos, Point targetPos)
        {
            ...
            while (openSet.Count > 0)
            {
                ...
                {
                    if (!CheckIfDiagonalsWalkable(grid, currentNode, neighbour))
                        continue;
        ...
        static bool CheckIfDiagonalsWalkable(Grid grid, Node node, Node neighbor)
        {
            var dx = neighbor.gridX - node.gridX;
            var dy = neighbor.gridY - node.gridY;

            bool isBlocked(int dx, int dy)
            {
                var newX = node.gridX + dx;
                var newY = node.gridY + dy;
                return newX >= 0 && newX < grid.nodes.GetLength(0) &&
                    newY >= 0 && newY < grid.nodes.GetLength(1) &&
                    !grid.nodes[newX, newY].walkable;
            }
